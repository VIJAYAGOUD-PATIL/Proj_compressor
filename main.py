...
class ScreenRecorder(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(hdr, text="  ⬛ Screen Recorder",
                 ...
                 bg="#7c3aed", fg="white", pady=14).pack(side="left", padx=6)
        ...
        tk.Label(info, text="  Records your entire screen using FFmpeg",
                 font=("Segoe UI",9), bg=self.CARD, fg=self.DIM,
                 pady=10, anchor="w").pack(fill="x")

        # Settings
        sets = tk.Frame(body, bg=self.BG)
        sets.pack(fill="x", pady=(0,12))

        # FPS
        fps_f = tk.Frame(sets, bg=self.BG)
        fps_f.pack(side="left", padx=(0,16))
        tk.Label(fps_f, text="FPS", font=("Segoe UI",8,"bold"),
                 bg=self.BG, fg=self.DIM).pack()
        self._fps_var = tk.StringVar(value="30")
        fps_cb = ttk.Combobox(fps_f, textvariable=self._fps_var,
                              values=["15","24","30","60"], width=6,
                              state="readonly")
        fps_cb.pack()

        # Monitor (for multi-monitor)
        mon_f = tk.Frame(sets, bg=self.BG)
        mon_f.pack(side="left", padx=(0,16))
        tk.Label(mon_f, text="Monitor", font=("Segoe UI",8,"bold"),
                 bg=self.BG, fg=self.DIM).pack()
        ...
        self._timer_lbl = tk.Label(st, text="00:00",
                 font=("Segoe UI",18,"bold"), bg=self.BG, fg="#7c3aed")
        ...
        self._dot = self._dot_cv.create_oval(2,2,14,14,
    ...
    def _start_recording(self):
        ...
        self._status_lbl.config(text="Recording screen…", fg=self.RED)
    ...
    def _stop_recording(self):
        ...
        if self._proc:
            ...
            self._proc = None
        self._rec_btn.config(text="  ● Start Recording  ", bg="#7c3aed")
